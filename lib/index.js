...
const temp_img_1 = __importDefault(require("./temp/temp_img"));
const server_tool_1 = __importDefault(require("./server_tool"));
exports.name = 'dew-bot';
const path = require('path');
exports.inject = ['puppeteer'];
exports.Config = koishi_1.Schema.object({
    wss: koishi_1.Schema.boolean().default(false).description('通常无需修改此项'),
    ...
    服务器地址: koishi_1.Schema.string().default('139.159.214.249'),
    图片压缩服务器: koishi_1.Schema.string().default('139.159.214.249'),
    // 139.159.214.249
});
async function apply(ctx, config) {
    ...
    ctx.on('ready', async () => {
        if (!server_1.default.init) {
            await server_1.default.setWsUrl(`${exports.CFG.wss ? 'wss' : 'ws'}://${exports.CFG.服务器地址}:8848`);
        }
        server_tool_1.default.setApiUrl(`${exports.CFG.图片压缩服务器}:8849`);
