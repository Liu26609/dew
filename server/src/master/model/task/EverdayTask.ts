...
export class EverdayTask extends baseCalss {
    createTime: number = 0;
    ...
    update(active: player) {
        ...
        const cfg = app.getCfg(xlsxCfgNames.每日任务表)
        ...
        this.baseGift = Math.ceil(this.baseGift) * this.list.length;
        this.gift_exp = 10 * god.getServerCfgItem('leve_exp_base').b;
    ...
    tick(k: taskKey, acitve: player,val:number) {
        ...
        if (this.end) {
            ...
            acitve.log.add(rank_type.每日任务完成次数, 1)
